  ...
  flake.modules.darwin.homebrew = _: {
    homebrew = {
      ...
      */
      ...
      /*
      Upgrade every cask to its latest Homebrew version on each switch, including
      the ones that update themselves (most of them). Without greedy, brew bundle
      skips any cask marked `auto_updates` or versioned `latest`.
      Trade-offs: switches are no longer idempotent; once an app self-updates,
      brew's recorded version goes stale, so it may re-download an already-current
      app, or briefly roll back one whose in-app channel ships ahead of the cask.
      A running app being upgraded may be quit by its cask's uninstall step.
      */
      greedyCasks = true;

      onActivation = {
        autoUpdate = true; # Whether to enable Homebrew to auto-update itself and all formulae during nix-darwin system activation. Needed so upgrade sees fresh cask metadata; brew itself stays pinned by nix-homebrew.
        /*
        ...
        */
        ...
        upgrade = true; # Whether to enable Homebrew to upgrade outdated formulae and casks during nix-darwin system activation (drops brew bundle's --no-upgrade).
      };
