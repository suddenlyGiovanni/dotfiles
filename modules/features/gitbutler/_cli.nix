# GitButler `but` CLI.
#
# The nixpkgs `gitbutler` package runs `cargo tauri build`, which compiles
# ONLY the GUI crate (`gitbutler-tauri`) — the `but` CLI crate is never built
# or installed. This derivation builds `but` (plus the `gitbutler-git` push/
# fetch helpers) from the same source tree.
#
# Vendored verbatim from nixpkgs PR #509081 ("gitbutler-cli: init at 0.19.7"):
#   https://github.com/NixOS/nixpkgs/pull/509081
# Once that PR merges, delete this file and use `pkgs.gitbutler-cli` instead.
# Keep `version` + the two hashes in sync with `pkgs.gitbutler` on each bump.
{
  lib,
  cmake,
  dbus,
