...
- **Code search routing**: structural questions ("find all callers of X", "find functions shaped Y", "rename across codebase") use `ast-grep` (binary: `sg`), not `rg`. Run `sg --help` if unsure of pattern syntax. Use `rg` for literal strings, error messages, log lines, comments, config values — not for identifiers. If a structural query needs `ast-grep` but it's not installed, say so; do not silently fall back to `rg`.
