...
description: Thermo-nuclear branch audit (bugs, breaking changes, security, devex, feature-flag leaks) scoped to the diff. Invoked via the Agent tool after a parent gathers diff and file contents. Loads rubric from the thermo-nuclear-review skill.
---
...
Typical flow: gather context first — run `git diff <base>...HEAD` (default base `main`) with the Bash tool, and read the full contents of the changed files directly (use the `Explore` agent for large change sets). Then invoke this agent via the Agent tool with `subagent_type: "thermo-nuclear-review-subagent"` and a user prompt containing `### Git / diff output` and `### Changed file contents`.
